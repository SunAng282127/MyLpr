...
def train():
    np.random.seed(1337)
    # 获取数据集
    X_train, y_train = generateds_ch.keras_read(num=45800, isTrain=True)
    X_test, y_test = generateds_ch.keras_read(num=29000, isTrain=False)
...
def predict(img):
    # 加载模型h5文件
    model = load_model('./model/model_ch.h5')
    # 预测模型
...
if __name__ == '__main__':
    # train()
    path = "./data_jpg/test_jpg/6/10_6.jpg"
    predict(path)
